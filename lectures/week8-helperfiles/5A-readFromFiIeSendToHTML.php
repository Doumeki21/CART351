...
$(document).on("click",getDataOnClickGET);

function getDataOnClickGET(){
  /* IS ALSO AJAX ...*/
  /*https://api.jquery.com/jquery.get/*/
$.get('files/testInput.txt',
    function(data) {
      //comes back unstructured and not as JSON....
      console.log(data);
      //so here need to break apart the string - and parse here ...
      //use split() and delimit on newline ..
...
function showResults(arrayFromServer){
  $(".wrapper-flex").empty();
  for(let i =0; i<arrayFromServer.length;i+=3){
     //**** need to split every string here on : ***/

    let container = $("<div>");
    $(container).addClass("single_container");
    let h1 = $("<h1>");
    h1.html("<span>ANIMAL:</span>"+arrayFromServer[i].split(":")[1]);
    $(container).append(h1);

    let h2 = $("<h2>");
    h2.html("<span>COLOR:</span>"+arrayFromServer[i+1].split(":")[1]);
    $(container).append(h2);

    let h3 = $("<h3>");
    h3.html("<span>NUMBER:</span>"+arrayFromServer[i+2].split(":")[1]);
    $(container).append(h3);
